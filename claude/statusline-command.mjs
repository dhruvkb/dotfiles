...
async function git(cwd, subcommand) {
	try {
		const {stdout} = await $({env: {...process.env, GIT_OPTIONAL_LOCKS: '0'}})`git -C ${cwd} ${subcommand}`
...
const rightParts = [chalk.yellow.bold(input.model.display_name ?? ''), ...rightSegments.filter(Boolean).map(wrap)]
